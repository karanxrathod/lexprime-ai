...
async function translateToEnglish(text: string): Promise<string> {
  ...
  const prompt = `You are an expert legal translator. Translate the following legal document text into clear, professional English. Maintain strict accuracy, legal terminology, and original formatting.\n\nText to translate:\n${text}`;
...
export async function analyzeDocumentWithGemini(params: AnalyzeParams): Promise<DocumentAnalysis> {
  ...
  if (hasDevanagari(content)) {
    try {
      // eslint-disable-next-line no-console
      console.log('[Gemini] Detected Devanagari script. Translating to English...');
      contentToAnalyze = await translateToEnglish(content);
      translatedContent = contentToAnalyze;
    } catch (e) {
      console.warn('[Gemini] Translation failed, using original text', e);
    }
  }

  const chunks = splitTextIntoChunks(contentToAnalyze, 4000, 400);
  const model = genAI.getGenerativeModel({ model: MODEL_NAME });
...
function buildChunkPrompt(content: string, language: 'en' | 'hi' | 'mr', level: SimplificationLevel, index: number, total: number): string {
  return [
    ...
    '',
    `- Language for explanations: ${language === 'hi' ? 'Hindi' : language === 'mr' ? 'Marathi' : 'English'}.`,
...
function buildSummaryPrompt(input: { language: 'en' | 'hi' | 'mr'; level: SimplificationLevel; clauses: any[]; risks: any[]; }): string {
  return [
    ...
    `Language: ${input.language === 'hi' ? 'Hindi' : input.language === 'mr' ? 'Marathi' : 'English'}. Tone: ${input.level === 'eli5' ? 'very simple' : input.level === 'simple' ? 'clear, non-legal' : 'concise professional'}.`,
    '',
    'Clauses:',
...
export async function chatWithGemini(req: ChatRequest): Promise<ChatMessage> {
  ...
  const systemPreamble = [
    'You are LexPrime AI, a helpful legal assistant that answers questions strictly based on the provided document content and the ongoing conversation history.',
    'Behavior rules:',
  ...
  const userPrompt = [
    systemPreamble,
    '',
    `Language: ${req.language === 'hi' ? 'Hindi' : req.language === 'mr' ? 'Marathi' : 'English'}`,
    '',
...
export async function analyzeClauseEnforceabilityWithGemini(params: EnforceabilityParams): Promise<ClauseEnforceabilityResult> {
  ...
  const prompt = [
    ...
    '}',
    '',
    `- Language for outputs (simplifiedMeaning, jurisdictionNotes): ${language === 'hi' ? 'Hindi' : language === 'mr' ? 'Marathi' : 'English'}.`,
...
export async function generateVisualizationsWithGemini(params: VisualizationParams): Promise<VisualizationBundle> {
  ...
  const prompt = [
    ...
    '',
    `- Language for textSummary and descriptions: ${language === 'hi' ? 'Hindi' : language === 'mr' ? 'Marathi' : 'English'}.`,
...
export async function analyzeDocumentAuthenticity(content: string, language: 'en' | 'hi' | 'mr'): Promise<AuthenticityAnalysis> {
  const genAI = getGenAI();
  if (!genAI) {
    throw new Error('Missing Gemini API key. Please configure it in Settings.');
  }
  ...
  const prompt = [
    ...
    '}',
    '',
    `- Language for text fields: ${language === 'hi' ? 'Hindi' : language === 'mr' ? 'Marathi' : 'English'}.`,
